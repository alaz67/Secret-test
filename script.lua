...
local State = {
    ...
    keybinds = {
        ...
        laggerMode = Enum.KeyCode.C,
        carrySpeed = Enum.KeyCode.V,
        normalSpeed = Enum.KeyCode.B,
        tpDown = Enum.KeyCode.N,
    ...
    -- CONTROLLER KEYBINDS
    controllerKeybinds = {
        drop = Enum.KeyCode.ButtonA,
        batAimbot = Enum.KeyCode.ButtonB,
        laggerMode = Enum.KeyCode.ButtonX,
        carrySpeed = Enum.KeyCode.ButtonY,
        normalSpeed = Enum.KeyCode.ButtonL1,
        tpDown = Enum.KeyCode.ButtonR1,
        autoLeft = Enum.KeyCode.DPadLeft,
        autoRight = Enum.KeyCode.DPadRight,
        instantReset = Enum.KeyCode.ButtonL3,
    },
...
local setAimbot, setAutoSwing, setBatCounter, setInstaGrab, setInfJump, setAntiRag, setFps, setMedusaCounter, setUnwalkToggle, setAntiBatBypass, setRagdollTimer
...
local CANDY_SKY_TAG = "SecretDuelSkyTheme"
...
local function enableAutoExitDuel()
    ...
    for _, v in ipairs(game:GetDescendants()) do watchAutoExitDuel(v) end
...
local function disableAutoExitDuel()
    ...
    autoExitLeaving = false
    for _, conn in ipairs(autoExitConnections) do pcall(function() conn:Disconnect() end) end
    autoExitConnections = {}
end

local function toggleAutoExitDuel()
    if autoExitDuelEnabled then disableAutoExitDuel() else enableAutoExitDuel() end
end
...
local function startRagdollTimer()
    ...
    LP.CharacterAdded:Connect(function(char) task.wait(0.5); setupRagdollBillboard(char) end)
...
local function stopRagdollTimer()
    if Conns.ragdollTimer then Conns.ragdollTimer:Disconnect(); Conns.ragdollTimer = nil end
    hitCountdownActive = false
    if hitCountdownLabel then hitCountdownLabel.Visible = false; hitCountdownLabel.Text = "" end
end
...
local function EnableAntiRagdoll()
    ...
    local respawnConn = LP.CharacterAdded:Connect(function() isBoosting = false; task.wait(0.5); cacheCharacterData() end)
...
local function playSong(soundParent, index, volume)
    ...
    local success, asset = pcall(function()
        if not State.cachedSongs[index] then downloadSong(index) end
...
function playPreview(idx)
    pcall(function()
        if previewSound then previewSound:Stop(); previewSound:Destroy(); previewSound = nil end
        local sound = playSong(CoreGui, idx, 0.3)
        if sound then
            previewSound = sound
            task.delay(4, function() if previewSound then previewSound:Stop(); previewSound:Destroy(); previewSound = nil end end)
        end
    end)
end
...
function playIntroAnimation()
    ...
    if not State.cachedSongs[State.selectedMusic] then
        ...
        if not success then print("Failed to download song!"); return end
    end
    ...
    task.spawn(function()
        ...
        pcall(function() game:GetService("ContentProvider"):PreloadAsync({SOUL_LOGO_ASSET_ID}) end)
        task.wait(0.3)
        local camera = workspace.CurrentCamera
...
local function startAntiBatBypass()
    ...
    Conns.antiBatBypass = RunService.Heartbeat:Connect(function()
        ...
        if root.AssemblyAngularVelocity.Magnitude > 0.3 then root.AssemblyAngularVelocity = Vector3.zero end
...
local function stopAntiBatBypass()
    if Conns.antiBatBypass then Conns.antiBatBypass:Disconnect(); Conns.antiBatBypass = nil end
end

local function restartAntiBatBypass()
    if State.antiBatBypassEnabled then stopAntiBatBypass(); startAntiBatBypass() end
end
...
local function startSpeedBypass()
    ...
    Conns.speedBypass = RunService.RenderStepped:Connect(function()
        ...
        if lagAmount > 0 then local t = tick(); while tick() - t < lagAmount do end end
    end)
end

local function stopSpeedBypass()
    if Conns.speedBypass then Conns.speedBypass:Disconnect(); Conns.speedBypass = nil end
end
...
local function onAnchorChanged(part)
    return part:GetPropertyChangedSignal("Anchored"):Connect(function()
        if part.Anchored and part.Transparency==1 then checkMedusaForInstantReset() end
    end)
end

local function setupMedusaDetection(char)
    if not char then return end
    for _, part in ipairs(char:GetDescendants()) do if part:IsA("BasePart") then onAnchorChanged(part) end end
    char.DescendantAdded:Connect(function(part) if part:IsA("BasePart") then onAnchorChanged(part) end end)
end
    ...
    if State.simpleFovEnabled then
        ...
        if not simpleFovConn then
            simpleFovConn = RunService.RenderStepped:Connect(function()
                if State.simpleFovEnabled then workspace.CurrentCamera.FieldOfView = State.simpleFovValue end
            end)
        end
    else
        if simpleFovConn then simpleFovConn:Disconnect(); simpleFovConn = nil end
...
local SkyOrder = {"Off","Night","Aurora","Sunset","Galaxy","Cyber","Sakura","Blood Moon","Emerald Dawn","Arctic","Vaporwave","Solar Eclipse","Heaven","Inferno"}
...
local function candySaveOriginalLighting()
    ...
    candyOriginalLighting = {ClockTime = Lighting.ClockTime, OutdoorAmbient = Lighting.OutdoorAmbient, Ambient = Lighting.Ambient, Brightness = Lighting.Brightness}
end
...
local function CandyApplyCustomSky(mode)
    ...
    if not preset or preset.kind == "off" then
        if candyOriginalLighting then for k, v in pairs(candyOriginalLighting) do pcall(function() Lighting[k] = v end) end end
...
local function getStickyTarget(currentRoot)
    ...
    if currentRoot and currentRoot.Parent then
        ...
        if currentPlr and hum and hum.Health > 0 then
            ...
            if currentPlr == newPlr or newDist > currentDist * 0.7 then return currentRoot, currentPlr end
        end
    end
...
local function swingCurrentBat(char)
    ...
    local bat = findBat()
    if bat and bat.Parent == char and bat:IsA("Tool") then pcall(function() bat:Activate() end) end
end
...
local function startBatAimbot()
    ...
    Conns.aimbot = RunService.RenderStepped:Connect(function()
        ...
        if not target then _aimbotTarget = nil; _aimbotTargetPlr = nil; swingCurrentBat(char); return end
        _aimbotTarget = target; _aimbotTargetPlr = targetPlr
...
LP.CharacterAdded:Connect(function(char)
    ...
    task.wait(1)
    if State.speedBypassEnabled then restartSpeedBypass() end
    if State.antiBatBypassEnabled then restartAntiBatBypass() end
    if State.ragdollTimerEnabled then task.wait(0.5); setupRagdollBillboard(char) end
end)
...
buildPage("Bat Aimbot",function()
    ...
    setAimbot=makeToggleRow("Bat Aimbot",State.batAimbotToggled,function(on) 
        if on then 
            if State.autoLeftEnabled then State.autoLeftEnabled=false; stopAutoLeft(); if stackBtnRefs.autoLeft then stackBtnRefs.autoLeft.setOn(false) end end
            if State.autoRightEnabled then State.autoRightEnabled=false; stopAutoRight(); if stackBtnRefs.autoRight then stackBtnRefs.autoRight.setOn(false) end end
            pcall(startBatAimbot) 
    ...
    makeGap(2)
    setAutoSwing=makeToggleRow("Auto Swing",State.autoSwingEnabled,function(on) State.autoSwingEnabled=on end)
    makeGap(2)
    setBatCounter=makeToggleRow("Bat Counter",State.batCounterEnabled,function(on) State.batCounterEnabled=on; if on then startBatCounter() else stopBatCounter() end end)
    makeGap(8)
...
buildPage("Mechanics",function()
    ...
    setInstaGrab=makeToggleRow("Auto Steal",Steal.AutoStealEnabled,function(on) Steal.AutoStealEnabled=on; if on then pcall(startAutoSteal) else stopAutoSteal() end end)
    makeGap(2)
    ...
    setInfJump=makeToggleRow("Infinite Jump (Hold)",State.holdInfJumpEnabled,function(on) 
        State.infJumpEnabled=on; State.holdInfJumpEnabled=on
        if on then startHoldInfJump() else stopHoldInfJump() end
    end)
    makeGap(2)
    setAntiRag=makeToggleRow("Anti Ragdoll (V1)",State.antiRagdollEnabled,function(on) 
    ...
    setRagdollTimer=makeToggleRow("Ragdoll Timer",State.ragdollTimerEnabled,function(on)
        ...
        if on then startRagdollTimer() else stopRagdollTimer() end
        task.spawn(function() if saveConfig then pcall(saveConfig) end end)
    end, "Shows 3-2-1-GO! countdown when ragdolled")
    makeGap(2)
    setFps=makeToggleRow("FPS Boost",State.fpsBoostEnabled,function(on) State.fpsBoostEnabled=on; if on then pcall(applyFPSBoost) end end)
    makeGap(2)
    setMedusaCounter=makeToggleRow("Medusa Counter",State.medusaCounterEnabled,function(on) State.medusaCounterEnabled=on; if on then setupMedusaCounter(LP.Character) else stopMedusaCounter() end end)
    makeGap(2)
    setUnwalkToggle=makeToggleRow("Unwalk",State.unwalkEnabled,function(on) State.unwalkEnabled=on; if on then startUnwalk() else stopUnwalk() end end)
    makeGap(2)
    setAntiBatBypass=makeToggleRow("Anti Bat Bypass",State.antiBatBypassEnabled,function(on) 
        State.antiBatBypassEnabled=on
        if on then startAntiBatBypass() else stopAntiBatBypass() end
        task.spawn(function() if saveConfig then pcall(saveConfig) end end)
    ...
    setAutoTPDownToggle=makeToggleRow("Auto TP Down",State.autoTPDownEnabled,function(on) State.autoTPDownEnabled=on; if on then startAutoTPDown() else stopAutoTPDown() end end)
    makeGap(2)
    local tpYBox=makeInputRow("TP Y Position",math.abs(State.autoTPDownY),function(n) if n and n>0 then State.autoTPDownY = -math.abs(n) end end)
...
buildPage("Visual",function()
    ...
    local fovToggle = makeToggleRow("FOV (110)",State.fovOn,function(on) State.fovOn = on; camera.FieldOfView = on and 110 or 70 end)
    makeGap(2)
    local galaxyToggle = makeToggleRow("Galaxy Sky",State.galaxyOn,function(on) State.galaxyOn = on; updateGalaxy() end)
    makeGap(2)
    local antiBatToggle = makeToggleRow("Anti Bat (Spin)",State.antiBatOn,function(on) State.antiBatOn = on end)
    makeGap(8)
    makeSectionHeader("Simple FOV + Sky Theme")
    makeGap(4)
    local simpleFovToggle = makeToggleRow("Simple FOV",State.simpleFovEnabled,function(on) State.simpleFovEnabled = on; toggleSimpleFOV() end)
    makeGap(2)
    ...
    mkCorner(skyBtn,6)
    local skyIndex = 1
    for i, name in ipairs(SkyOrder) do if name == State.currentSkyTheme then skyIndex = i; break end end
...
buildPage("Keybinds",function()
    ...
    makeGap(8)
    makeSectionHeader("🎮 Controller Keybinds"); makeGap(4)
...
buildPage("Settings",function()
    ...
    setHideButtonsToggle=makeToggleRow("Hide Mobile Buttons",State.stackButtonsHidden,function(on) 
        State.stackButtonsHidden=on
        for _,wrapper in pairs(stackWrappers) do wrapper.Visible=not on end
        if mobBox then mobBox.Visible = not on end
        if mobBoxOverlay then mobBoxOverlay.Visible = not on end
    ...
    prevBtn.MouseButton1Click:Connect(function()
        ...
        task.spawn(function()
            local success = downloadSong(State.selectedMusic)
            if success then playPreview(State.selectedMusic) end
        end)
        task.spawn(function() if saveConfig then pcall(saveConfig) end end)
    end)
    ...
    playIntroBtn.MouseButton1Click:Connect(function()
        if not State.cachedSongs[State.selectedMusic] then
            task.spawn(function()
                local success = downloadSong(State.selectedMusic)
                if success then playIntroAnimation() end
            end)
        else
            playIntroAnimation()
        end
    ...
    local bypassToggle = makeToggleRow("Enable Speed Bypass",State.speedBypassEnabled,function(on)
        ...
        if on then startSpeedBypass() else stopSpeedBypass() end
        task.spawn(function() if saveConfig then pcall(saveConfig) end end)
    end)
    
    makeGap(2)
    
    -- Speed Bypass Power Slider
    ...
    powerSliderBtn.InputEnded:Connect(function(inp)
        if inp.UserInputType == Enum.UserInputType.MouseButton1 or inp.UserInputType == Enum.UserInputType.Touch then powerDragging = false end
    end)
    UIS.InputChanged:Connect(function(inp)
    ...
    local fl=Instance.new("TextLabel",fw); fl.Size=UDim2.new(1,0,1,0); fl.BackgroundTransparency=1; fl.Text="secret duel hub  ·  v2.0"; fl.TextColor3=Color3.fromRGB(55,55,55); fl.Font=Enum.Font.Gotham; fl.TextSize=9; fl.TextXAlignment=Enum.TextXAlignment.Center
end)
...
local mobBox=Instance.new("Frame",gui); mobBox.Name="SecretDuelMobBox"; mobBox.Size=UDim2.new(0,BOX_W,0,BOX_H); mobBox.Position=UDim2.new(1,-(BOX_W+10),0.5,-(BOX_H/2)); mobBox.BackgroundColor3=C.winBg; mobBox.BackgroundTransparency=0; mobBox.BorderSizePixel=0; mobBox.Active=true; mobBox.ZIndex=15; mkCorner(mobBox,12); mkStroke(mobBox,C.winBorder,2); makeDraggable(mobBox)
...
for i,def in ipairs(stackDefs) do
    ...
    btnFrame.MouseButton1Click:Connect(function()
        ...
        elseif def.key=="aimbot" then 
            if not ns then pcall(stopBatAimbot)
            else
                if State.autoLeftEnabled then State.autoLeftEnabled=false; stopAutoLeft(); if stackBtnRefs.autoLeft then stackBtnRefs.autoLeft.setOn(false) end end
                if State.autoRightEnabled then State.autoRightEnabled=false; stopAutoRight(); if stackBtnRefs.autoRight then stackBtnRefs.autoRight.setOn(false) end end
                pcall(startBatAimbot)
...
local function setupChar(char)
    ...
    if head then
        ...
        local bb=Instance.new("BillboardGui",head); bb.Name="SecretDuelBB"; bb.Size=UDim2.new(0,180,0,56); bb.StudsOffset=Vector3.new(0,3.2,0); bb.AlwaysOnTop=true
        ...
        local lbl2=Instance.new("TextLabel",bb); lbl2.Size=UDim2.new(1,0,0,18); lbl2.Position=UDim2.new(0,0,0,32); lbl2.BackgroundTransparency=1; lbl2.Text="/secretduel"; lbl2.TextColor3=Color3.fromRGB(200,200,200); lbl2.Font=Enum.Font.GothamBold; lbl2.TextScaled=true; lbl2.TextStrokeTransparency=0.1; lbl2.TextStrokeColor3=Color3.new(0,0,0)
    end
    ...
    DisableAntiRagdoll()
    if State.antiRagdollEnabled then task.wait(0.5); EnableAntiRagdoll() end
    if State.ragdollTimerEnabled then task.wait(0.5); setupRagdollBillboard(char) end
    ...
    if State.speedBypassEnabled then task.wait(0.5); restartSpeedBypass() end
    if State.antiBatBypassEnabled then task.wait(0.5); restartAntiBatBypass() end
end
...
task.spawn(function()
    task.wait(0.5)
    if State.ragdollTimerEnabled then startRagdollTimer() end
...
print("  [T] Instant Reset  [Q] Auto Left  [E] Auto Right")
print("  [X] Drop BR  [Z] Bat Aimbot  [C] Lagger Mode")
print("  [V] Carry Speed  [B] Normal Speed  [N] TP Down")
print("  [F] Toggle Tracers  [G] Toggle Medusa Reset")
print("")
