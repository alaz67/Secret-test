...
-- ADDED: Improved Keybind System from Candy Hub
...
-- CHANGED: Name to "Secret Duel Hub"
-- CHANGED: Logo to texture ID 6974075369
-- CHANGED: Background image to texture ID 6974075369
-- ============================================================
...
local LOGO_ID = "rbxassetid://6974075369"
local SOUL_LOGO_ASSET_ID = "rbxassetid://6974075369"
...
local State = {
    ...
    simpleFovValue=70,
    currentSkyTheme="Off",
    -- Keybinds (Keyboard) - improved system
    keybinds = {
...
local GAMEPAD_KEYS = {
    ...
    [Enum.KeyCode.ButtonL4] = true,
    [Enum.KeyCode.ButtonR4] = true,
    [Enum.KeyCode.ButtonL5] = true,
    [Enum.KeyCode.ButtonR5] = true,
    [Enum.KeyCode.ButtonL6] = true,
    [Enum.KeyCode.ButtonR6] = true,
}

local function isGamepadInput(input)
    if not input or not input.UserInputType then return false end
    return string.find(input.UserInputType.Name, "Gamepad") ~= nil
end

local function isControllerButton(keyCode)
    return GAMEPAD_KEYS[keyCode] == true
end
...
local function getControllerButtonName(keyCode)
    local names = {
        ...
        [Enum.KeyCode.DPadUp] = "DPAD↑",
        [Enum.KeyCode.DPadDown] = "DPAD↓",
        [Enum.KeyCode.DPadLeft] = "DPAD←",
        [Enum.KeyCode.DPadRight] = "DPAD→",
...
local ragdollCharAddedConn = nil
...
local function startRagdollTimer()
    ...
    if not ragdollCharAddedConn then
        ragdollCharAddedConn = LP.CharacterAdded:Connect(function(char)
            task.wait(0.5)
...
local function stopRagdollTimer()
    ...
    if ragdollCharAddedConn then
        ragdollCharAddedConn:Disconnect()
        ragdollCharAddedConn = nil
    end
    hitCountdownActive = false
...
local function downloadSong(index)
    return pcall(function()
        ...
        local cacheName = "SecretDuelSong_" .. index .. ".mp3"
...
function playIntroAnimation()
    ...
    secretLabel.Size = UDim2.new(0, 500, 0, 120)
    secretLabel.Position = UDim2.new(0, -450, 0.5, -100)
    secretLabel.AnchorPoint = Vector2.new(0.5, 0.5)
    secretLabel.BackgroundTransparency = 1
    secretLabel.Text = "SECRET"
    secretLabel.TextColor3 = Color3.fromRGB(0, 0, 0)
    secretLabel.TextTransparency = 0
    secretLabel.TextSize = 100
    secretLabel.Font = Enum.Font.GothamBold
    secretLabel.TextStrokeColor3 = Color3.fromRGB(0, 0, 0)
    secretLabel.TextStrokeTransparency = 1
    secretLabel.ZIndex = 2
    secretLabel.Parent = introFrame

    local duelLabel = Instance.new("TextLabel")
    duelLabel.Size = UDim2.new(0, 500, 0, 120)
    duelLabel.Position = UDim2.new(1, 450, 0.5, 100)
    duelLabel.AnchorPoint = Vector2.new(0.5, 0.5)
    duelLabel.BackgroundTransparency = 1
    duelLabel.Text = "DUEL"
    duelLabel.TextColor3 = Color3.fromRGB(0, 0, 0)
    duelLabel.TextTransparency = 0
    duelLabel.TextSize = 100
    duelLabel.Font = Enum.Font.GothamBold
    duelLabel.TextStrokeColor3 = Color3.fromRGB(0, 0, 0)
    duelLabel.TextStrokeTransparency = 1
    duelLabel.ZIndex = 2
    duelLabel.Parent = introFrame
    ...
    task.spawn(function()
        ...
        task.spawn(function()
            while flickering do
                ...
                logoImage.ImageTransparency = 0
                secretLabel.TextTransparency = 0.25
                secretLabel.TextStrokeTransparency = 0.3
                duelLabel.TextTransparency = 0.25
                duelLabel.TextStrokeTransparency = 0.3
                task.wait(0.08)
            end
        end)

        local tweenInfo = TweenInfo.new(1.5, Enum.EasingStyle.Quad, Enum.EasingDirection.Out)
        local secretTween = TS:Create(secretLabel, tweenInfo, {Position = UDim2.new(0.5, 0, 0.5, -100)})
        secretTween:Play()
        
        task.wait(0.55)
        
        local duelTween = TS:Create(duelLabel, tweenInfo, {Position = UDim2.new(0.5, 0, 0.5, 100)})
        duelTween:Play()
        
        secretTween.Completed:Wait()
        task.wait(0.5)
        ...
        TS:Create(secretLabel, fadeInfo, {TextTransparency = 1, TextStrokeTransparency = 1}):Play()
        TS:Create(duelLabel, fadeInfo, {TextTransparency = 1, TextStrokeTransparency = 1}):Play()
...
-- ========== IMPROVED KEYBIND SYSTEM (from Candy Hub) ==========
...
local function executeKeybindAction(action)
    if action == "drop" then
...
-- ========== HOLD INFINITE JUMP ==========
local holdInfJumpConns = {}

local function startHoldInfJump()
    if holdInfJumpConns.jump then return end
    ...
    holdInfJumpConns = {jump = jumpConn, heart = heartConn}
end

local function stopHoldInfJump()
    if holdInfJumpConns.jump then 
        holdInfJumpConns.jump:Disconnect() 
        holdInfJumpConns.jump = nil
    end
    if holdInfJumpConns.heart then 
        holdInfJumpConns.heart:Disconnect() 
        holdInfJumpConns.heart = nil
    end
    holdInfJumpConns = {}
end
...
local function candyClearSky()
    for _, child in ipairs(Lighting:GetChildren()) do
        if child:GetAttribute("SecretDuelSkyTheme") then
...
local function updateGalaxy()
    if State.galaxyOn then
        local sky = Lighting:FindFirstChild("SecretDuelGalaxySky") or Instance.new("Sky")
        sky.Name = "SecretDuelGalaxySky"
        ...
        if Lighting:FindFirstChild("SecretDuelGalaxySky") then Lighting.SecretDuelGalaxySky:Destroy() end
...
local uiScaleObj_inst = Instance.new("UIScale",gui)
uiScaleObj_inst.Scale=1.0

-- FLOATING BUTTON
local vBtnFrame = Instance.new("ImageButton",gui)
vBtnFrame.Name="SecretDuelPill"
vBtnFrame.Size=UDim2.new(0,48,0,48)
vBtnFrame.Position=UDim2.new(1,-58,0,14)
vBtnFrame.BackgroundColor3=Color3.fromRGB(0,0,0)
vBtnFrame.BackgroundTransparency=0.1
vBtnFrame.BorderSizePixel=0
vBtnFrame.Active=true
vBtnFrame.ZIndex=20
Instance.new("UICorner",vBtnFrame).CornerRadius=UDim.new(0,12)
Instance.new("UIStroke",vBtnFrame).Color=Color3.fromRGB(200,200,200)
Instance.new("UIStroke",vBtnFrame).Thickness=1.5
vBtnFrame.Image=LOGO_ID
vBtnFrame.ImageTransparency=0.1
...
mainOuter.BackgroundColor3=Color3.fromRGB(8,8,8)
mainOuter.BorderSizePixel=0
mainOuter.ClipsDescendants=true
Instance.new("UICorner",mainOuter).CornerRadius=UDim.new(0,10)
Instance.new("UIStroke",mainOuter).Color=Color3.fromRGB(220,220,220)
Instance.new("UIStroke",mainOuter).Thickness=2
...
vBtnFrame.MouseButton1Click:Connect(function()
    ...
    if State.guiVisible then 
        ...
        vBtnFrame.ImageTransparency=0.6
        vBtnFrame.BackgroundTransparency=0.5 
    end
end)

-- DRAG for floating button
...
vBtnFrame.MouseEnter:Connect(function()
    if State.guiVisible then
        TS:Create(vBtnFrame,TweenInfo.new(0.15),{BackgroundTransparency=0.05}):Play()
    else
        TS:Create(vBtnFrame,TweenInfo.new(0.15),{BackgroundTransparency=0.3}):Play()
    end
end)

vBtnFrame.MouseLeave:Connect(function()
    if State.guiVisible then
        TS:Create(vBtnFrame,TweenInfo.new(0.15),{BackgroundTransparency=0.1}):Play()
    else
        TS:Create(vBtnFrame,TweenInfo.new(0.15),{BackgroundTransparency=0.5}):Play()
    end
end)
...
bgImg.BackgroundColor3=Color3.fromRGB(8,8,8)
...
titleBar.BackgroundColor3=Color3.fromRGB(6,6,6)
...
titleAccent.BackgroundColor3=Color3.fromRGB(220,220,220)
...
Instance.new("UIStroke",closeBtn).Color=Color3.fromRGB(70,70,70)
...
Instance.new("UIStroke",lockBtn).Color=Color3.fromRGB(70,70,70)
Instance.new("UIStroke",lockBtn).Thickness=1
...
titleDiv.BackgroundColor3=Color3.fromRGB(50,50,50)
titleDiv.BorderSizePixel=0
titleDiv.ZIndex=5

-- TAB COLUMN
local tabCol=Instance.new("Frame",mainOuter)
tabCol.Size=UDim2.new(0,TAB_COL_W,1,-TITLE_H)
tabCol.Position=UDim2.new(0,0,0,TITLE_H)
tabCol.BackgroundColor3=Color3.fromRGB(8,8,8)
tabCol.BackgroundTransparency=0.12
tabCol.BorderSizePixel=0
tabCol.ZIndex=4

local tabColDiv=Instance.new("Frame",tabCol)
tabColDiv.Size=UDim2.new(0,1,1,0)
tabColDiv.Position=UDim2.new(1,-1,0,0)
tabColDiv.BackgroundColor3=Color3.fromRGB(50,50,50)
tabColDiv.BorderSizePixel=0
tabColDiv.ZIndex=5
...
contentArea.Size=UDim2.new(0,WIN_W-TAB_COL_W,1,-TITLE_H)
contentArea.Position=UDim2.new(0,TAB_COL_W,0,TITLE_H)
...
local function switchTab(name)
    ...
    for _,n in ipairs(TABS) do
        local t=tabBtns[n]
        local isA=(n==name)
        TS:Create(t.frame,TweenInfo.new(0.14),{BackgroundColor3=isA and Color3.fromRGB(45,45,45) or Color3.fromRGB(18,18,18)}):Play()
        TS:Create(t.lbl,TweenInfo.new(0.14),{TextColor3=isA and Color3.fromRGB(255,255,255) or Color3.fromRGB(120,120,120)}):Play()
        if tabPages[n] then
...
for i,name in ipairs(TABS) do
    ...
    btn.BackgroundColor3=(name==currentTab) and Color3.fromRGB(45,45,45) or Color3.fromRGB(18,18,18)
    ...
    lbl.BackgroundTransparency=1
    lbl.Text=name
    lbl.TextColor3=(name==currentTab) and Color3.fromRGB(255,255,255) or Color3.fromRGB(120,120,120)
    lbl.Font=Enum.Font.GothamBold
    lbl.TextSize=9
    ...
    btn.MouseEnter:Connect(function()
        if currentTab~=name then
            TS:Create(btn,TweenInfo.new(0.1),{BackgroundColor3=Color3.fromRGB(22,22,22)}):Play()
        end
    end)
    btn.MouseLeave:Connect(function()
        if currentTab~=name then
            TS:Create(btn,TweenInfo.new(0.1),{BackgroundColor3=Color3.fromRGB(18,18,18)}):Play()
        end
    end)
end
...
local function makeSectionHeader(label)
    ...
    wrap.BackgroundColor3=Color3.fromRGB(14,14,14)
    wrap.BackgroundTransparency=0.5
    wrap.BorderSizePixel=0
    wrap.LayoutOrder=LO()
    Instance.new("UICorner",wrap).CornerRadius=UDim.new(0,0)
...
local function makeInputRow(label,default,onChange)
    ...
    Instance.new("UIStroke",row).Color=Color3.fromRGB(45,45,45)
    Instance.new("UIStroke",row).Thickness=1
    
    row.MouseEnter:Connect(function()
        TS:Create(row,TweenInfo.new(0.1),{BackgroundColor3=Color3.fromRGB(22,22,22)}):Play()
    end)
    row.MouseLeave:Connect(function()
        TS:Create(row,TweenInfo.new(0.1),{BackgroundColor3=Color3.fromRGB(12,12,12)}):Play()
    end)
    ...
    boxWrap.Size=UDim2.new(0,66,0,24)
    boxWrap.Position=UDim2.new(1,-76,0.5,-12)
    boxWrap.BackgroundColor3=Color3.fromRGB(8,8,8)
    boxWrap.BorderSizePixel=0
    Instance.new("UICorner",boxWrap).CornerRadius=UDim.new(0,6)
    
    local bs=Instance.new("UIStroke",boxWrap)
    ...
    box.Focused:Connect(function()
        TS:Create(bs,TweenInfo.new(0.15),{Color=Color3.fromRGB(200,200,200)}):Play()
    end)
    
    box.FocusLost:Connect(function()
        TS:Create(bs,TweenInfo.new(0.15),{Color=Color3.fromRGB(60,60,60)}):Play()
        if onChange then
...
local function makeToggleRow(label,defaultOn,onToggle,subtext)
    local rowH=subtext and 52 or 40
    local row=Instance.new("Frame",currentPage)
    ...
    if subtext then
        local s2=Instance.new("TextLabel",row)
        s2.Size=UDim2.new(1,-68,0,14)
        s2.Position=UDim2.new(0,10,0,27)
        s2.BackgroundTransparency=1
        s2.Text=subtext
        s2.TextColor3=Color3.fromRGB(120,120,120)
    ...
    pillBg.BackgroundColor3=defaultOn and Color3.fromRGB(190,190,190) or Color3.fromRGB(35,35,35)
    pillBg.BorderSizePixel=0
    pillBg.ZIndex=7
    Instance.new("UICorner",pillBg).CornerRadius=UDim.new(0,10)
    Instance.new("UIStroke",pillBg).Color=Color3.fromRGB(90,90,90)
    Instance.new("UIStroke",pillBg).Thickness=1
    
    local dot=Instance.new("Frame",pillBg)
    dot.Size=UDim2.new(0,14,0,14)
    dot.Position=defaultOn and UDim2.new(1,-17,0.5,-7) or UDim2.new(0,3,0.5,-7)
    dot.BackgroundColor3=defaultOn and Color3.fromRGB(255,255,255) or Color3.fromRGB(70,70,70)
    ...
    local function setV(on)
        isOn=on
        TS:Create(pillBg,TweenInfo.new(0.18,Enum.EasingStyle.Quad),{BackgroundColor3=on and Color3.fromRGB(190,190,190) or Color3.fromRGB(35,35,35)}):Play()
        TS:Create(dot,TweenInfo.new(0.18,Enum.EasingStyle.Back),{
            Position=on and UDim2.new(1,-17,0.5,-7) or UDim2.new(0,3,0.5,-7),
            BackgroundColor3=on and Color3.fromRGB(255,255,255) or Color3.fromRGB(70,70,70)
        }):Play()
    end
...
buildPage("Speed",function()
    ...
    modeRow.BackgroundColor3=Color3.fromRGB(12,12,12)
    modeRow.BorderSizePixel=0
    modeRow.LayoutOrder=LO()
    Instance.new("UICorner",modeRow).CornerRadius=UDim.new(0,7)
    Instance.new("UIStroke",modeRow).Color=Color3.fromRGB(45,45,45)
    Instance.new("UIStroke",modeRow).Thickness=1
    
    local mLbl=Instance.new("TextLabel",modeRow)
    mLbl.Size=UDim2.new(0,60,1,0)
    mLbl.Position=UDim2.new(0,10,0,0)
    mLbl.BackgroundTransparency=1
    mLbl.Text="Mode"
    mLbl.TextColor3=Color3.fromRGB(240,240,240)
    mLbl.Font=Enum.Font.GothamBold
    mLbl.TextSize=11
    mLbl.TextXAlignment=Enum.TextXAlignment.Left
    
    local mValLbl=Instance.new("TextLabel",modeRow)
    mValLbl.Size=UDim2.new(0,70,1,0)
    mValLbl.Position=UDim2.new(0,70,0,0)
    mValLbl.BackgroundTransparency=1
    mValLbl.Text="Normal"
    mValLbl.TextColor3=Color3.fromRGB(120,120,120)
    mValLbl.Font=Enum.Font.GothamBold
    mValLbl.TextSize=11
    mValLbl.TextXAlignment=Enum.TextXAlignment.Left
    
    local mClk=Instance.new("TextButton",modeRow)
    mClk.Size=UDim2.new(0.65,0,1,0)
    mClk.BackgroundTransparency=1
    mClk.Text=""
    mClk.ZIndex=6
    mClk.MouseButton1Click:Connect(function()
        State.speedToggled=not State.speedToggled
        mValLbl.Text=State.speedToggled and "Carry" or "Normal"
        if stackBtnRefs.carrySpeed then stackBtnRefs.carrySpeed.setOn(State.speedToggled) end
        task.spawn(function()
            if saveConfig then pcall(saveConfig) end
        end)
    ...
    makeGap(8)
    makeSectionHeader("Bat Aimbot Speed")
    makeGap(4)
    makeInputRow("Aimbot Speed",State.batAimbotSpeed,function(n)
        if n>=10 and n<=200 then State.batAimbotSpeed=n end
    end)
end)
...
buildPage("Bat Aimbot",function()
    makeGap(4)
    makeSectionHeader("Candy Bat Aimbot")
    makeGap(4)
    ...
    setAutoSwing=makeToggleRow("Auto Swing",true,function(on)
    ...
    infoLabel.Text="⚔️ Candy Bat Aimbot\nSticky targeting • Adaptive prediction • Auto-swing"
...
buildPage("Mechanics",function()
    ...
    makeInputRow("Steal Duration",Steal.StealDuration,function(n)
        if n>=0.05 and n<=5 then Steal.StealDuration=n end
    end)
    makeGap(8)
    ...
    bypassRow.BackgroundColor3=Color3.fromRGB(12,12,12)
    bypassRow.BorderSizePixel=0
    bypassRow.LayoutOrder=LO()
    Instance.new("UICorner",bypassRow).CornerRadius=UDim.new(0,7)
    Instance.new("UIStroke",bypassRow).Color=Color3.fromRGB(45,45,45)
    Instance.new("UIStroke",bypassRow).Thickness=1
    ...
    threshLbl.TextColor3=Color3.fromRGB(240,240,240)
    ...
    threshBox.BackgroundColor3=Color3.fromRGB(8,8,8)
    ...
    Instance.new("UIStroke",threshBox).Color=Color3.fromRGB(60,60,60)
    Instance.new("UIStroke",threshBox).Thickness=1
    ...
    capLbl.TextColor3=Color3.fromRGB(240,240,240)
    ...
    capBox.BackgroundColor3=Color3.fromRGB(8,8,8)
    ...
    Instance.new("UIStroke",capBox).Color=Color3.fromRGB(60,60,60)
    Instance.new("UIStroke",capBox).Thickness=1
    ...
    bypassInfo.Text="Threshold: speed to detect • Cap: max speed limit"
    bypassInfo.TextColor3=Color3.fromRGB(120,120,120)
    ...
    local tpYBox=makeInputRow("TP Y Position",20,function(n)
        if n and n>0 then State.autoTPDownY = -math.abs(n) end
    end)
    makeGap(8)
    
    makeSectionHeader("Tracers")
    makeGap(4)
    
    local tracerToggle = makeToggleRow("Enable Tracers",tracersEnabled,function(on)
        tracersEnabled=on
        if not on then clearTracers() end
    end)
    makeGap(2)
    
    local tracerInfo = Instance.new("TextLabel",currentPage)
    tracerInfo.Size=UDim2.new(1,0,0,20)
    tracerInfo.BackgroundTransparency=1
    tracerInfo.Text="Press F to toggle Tracers"
    tracerInfo.TextColor3=Color3.fromRGB(120,120,120)
    tracerInfo.Font=Enum.Font.Gotham
    tracerInfo.TextSize=10
    tracerInfo.TextXAlignment=Enum.TextXAlignment.Center
    tracerInfo.LayoutOrder=LO()
    makeGap(8)
    
    makeSectionHeader("Instant Reset on Medusa")
    makeGap(4)
    
    local medusaToggle = makeToggleRow("Auto Reset on Medusa",State.instantResetOnMedusa,function(on)
        State.instantResetOnMedusa=on
    end)
    makeGap(2)
    
    local medusaInfo = Instance.new("TextLabel",currentPage)
    medusaInfo.Size=UDim2.new(1,0,0,20)
    medusaInfo.BackgroundTransparency=1
    medusaInfo.Text="Medusa auto-reset uses main Instant Reset"
    medusaInfo.TextColor3=Color3.fromRGB(120,120,120)
    medusaInfo.Font=Enum.Font.Gotham
    medusaInfo.TextSize=10
    medusaInfo.TextXAlignment=Enum.TextXAlignment.Center
    medusaInfo.LayoutOrder=LO()
    makeGap(8)
    ...
    resetRow.Size=UDim2.new(1,0,0,45)
    resetRow.BackgroundColor3=Color3.fromRGB(12,12,12)
    resetRow.BorderSizePixel=0
    resetRow.LayoutOrder=LO()
    Instance.new("UICorner",resetRow).CornerRadius=UDim.new(0,7)
    Instance.new("UIStroke",resetRow).Color=Color3.fromRGB(45,45,45)
    Instance.new("UIStroke",resetRow).Thickness=1
    
    local resetBtn=Instance.new("TextButton",resetRow)
    resetBtn.Size=UDim2.new(1,-20,1,-8)
    resetBtn.Position=UDim2.new(0,10,0,4)
    resetBtn.BackgroundColor3=Color3.fromRGB(200,50,50)
    resetBtn.BorderSizePixel=0
    resetBtn.Text="INSTANT RESET (T Key)"
    ...
    resetBtn.MouseButton1Click:Connect(function()
        performInstantReset()
        TS:Create(resetBtn,TweenInfo.new(0.1),{BackgroundColor3=Color3.fromRGB(255,0,0)}):Play()
        task.delay(0.15,function()
            TS:Create(resetBtn,TweenInfo.new(0.1),{BackgroundColor3=Color3.fromRGB(200,50,50)}):Play()
        end)
    end)
end)
...
buildPage("Visual",function()
    ...
    fovSliderRow.BackgroundColor3=Color3.fromRGB(12,12,12)
    fovSliderRow.BorderSizePixel=0
    fovSliderRow.LayoutOrder=LO()
    Instance.new("UICorner",fovSliderRow).CornerRadius=UDim.new(0,7)
    Instance.new("UIStroke",fovSliderRow).Color=Color3.fromRGB(45,45,45)
    Instance.new("UIStroke",fovSliderRow).Thickness=1
    ...
    fovSliderLbl.TextColor3=Color3.fromRGB(240,240,240)
    ...
    Instance.new("UICorner",fovSliderBg).CornerRadius=UDim.new(0,3)
    ...
    Instance.new("UICorner",fovSliderFill).CornerRadius=UDim.new(0,3)
    ...
    skyRow.BackgroundColor3=Color3.fromRGB(12,12,12)
    skyRow.BorderSizePixel=0
    skyRow.LayoutOrder=LO()
    Instance.new("UICorner",skyRow).CornerRadius=UDim.new(0,7)
    Instance.new("UIStroke",skyRow).Color=Color3.fromRGB(45,45,45)
    Instance.new("UIStroke",skyRow).Thickness=1
    ...
    skyLbl.TextColor3=Color3.fromRGB(240,240,240)
...
-- ============================================================
-- KEYBINDS PAGE (with Controller Buttons - improved)
-- ============================================================
buildPage("Keybinds",function()
    makeGap(4)
    makeSectionHeader("Keyboard Keybinds")
    makeGap(4)
    ...
    for _, data in ipairs(keybindActions) do
        ...
        Instance.new("UIStroke",keyBtn).Color=Color3.fromRGB(60,60,70)
        Instance.new("UIStroke",keyBtn).Thickness=1
    ...
    makeGap(8)
    makeSectionHeader("🎮 Controller Keybinds")
    makeGap(4)
    ...
    for _, data in ipairs(controllerActions) do
        ...
        keyBtn.TextSize=11
        Instance.new("UICorner",keyBtn).CornerRadius=UDim.new(0,6)
        Instance.new("UIStroke",keyBtn).Color=Color3.fromRGB(50,80,120)
        Instance.new("UIStroke",keyBtn).Thickness=1
    ...
    layoutInfo.BackgroundColor3=Color3.fromRGB(20,25,35)
    layoutInfo.BorderSizePixel=0
    layoutInfo.LayoutOrder=LO()
    Instance.new("UICorner",layoutInfo).CornerRadius=UDim.new(0,7)
    Instance.new("UIStroke",layoutInfo).Color=Color3.fromRGB(60,70,90)
    Instance.new("UIStroke",layoutInfo).Thickness=1
    ...
    layoutLbl.Text = [[🎮 CONTROLLER LAYOUT:
    ...
    resetLabel.TextColor3=Color3.fromRGB(240,240,240)
    ...
    Instance.new("UICorner",resetKbBtn).CornerRadius=UDim.new(0,6)
...
buildPage("Settings",function()
    ...
    makeGap(8)
    makeSectionHeader("Save / Load")
    makeGap(4)
    
    local saveRow = Instance.new("Frame",currentPage)
    saveRow.Size=UDim2.new(1,0,0,45)
    saveRow.BackgroundColor3=Color3.fromRGB(12,12,12)
    saveRow.BorderSizePixel=0
    saveRow.LayoutOrder=LO()
    Instance.new("UICorner",saveRow).CornerRadius=UDim.new(0,7)
    Instance.new("UIStroke",saveRow).Color=Color3.fromRGB(45,45,45)
    Instance.new("UIStroke",saveRow).Thickness=1
    ...
    saveLbl.TextColor3=Color3.fromRGB(240,240,240)
    ...
    Instance.new("UIStroke",saveBtn).Color=Color3.fromRGB(0,200,0)
    Instance.new("UIStroke",saveBtn).Thickness=1
    ...
    makeGap(8)
    makeSectionHeader("Auto Exit Duel")
    makeGap(4)
    
    local autoExitRow = Instance.new("Frame",currentPage)
    autoExitRow.Size=UDim2.new(1,0,0,45)
    autoExitRow.BackgroundColor3=Color3.fromRGB(12,12,12)
    autoExitRow.BorderSizePixel=0
    autoExitRow.LayoutOrder=LO()
    Instance.new("UICorner",autoExitRow).CornerRadius=UDim.new(0,7)
    Instance.new("UIStroke",autoExitRow).Color=Color3.fromRGB(45,45,45)
    Instance.new("UIStroke",autoExitRow).Thickness=1
    ...
    autoExitLbl.TextColor3=Color3.fromRGB(240,240,240)
    ...
    autoExitPillBg.BackgroundColor3=State.autoExitDuelEnabled and Color3.fromRGB(190,190,190) or Color3.fromRGB(35,35,35)
    autoExitPillBg.BorderSizePixel=0
    autoExitPillBg.ZIndex=7
    Instance.new("UICorner",autoExitPillBg).CornerRadius=UDim.new(0,10)
    Instance.new("UIStroke",autoExitPillBg).Color=Color3.fromRGB(90,90,90)
    Instance.new("UIStroke",autoExitPillBg).Thickness=1
    ...
    autoExitDot.BackgroundColor3=State.autoExitDuelEnabled and Color3.fromRGB(255,255,255) or Color3.fromRGB(70,70,70)
    autoExitDot.BorderSizePixel=0
    autoExitDot.ZIndex=8
    Instance.new("UICorner",autoExitDot).CornerRadius=UDim.new(0,7)
    ...
    autoExitClk.MouseButton1Click:Connect(function()
        ...
        TS:Create(autoExitPillBg,TweenInfo.new(0.18,Enum.EasingStyle.Quad),{BackgroundColor3=isOn and Color3.fromRGB(190,190,190) or Color3.fromRGB(35,35,35)}):Play()
        TS:Create(autoExitDot,TweenInfo.new(0.18,Enum.EasingStyle.Back),{
            ...
            BackgroundColor3=isOn and Color3.fromRGB(255,255,255) or Color3.fromRGB(70,70,70)
        }):Play()
        task.spawn(function()
            if saveConfig then pcall(saveConfig) end
        end)
    ...
    autoExitInfo.TextColor3=Color3.fromRGB(120,120,120)
    ...
    introToggleRow.BackgroundColor3=Color3.fromRGB(12,12,12)
    introToggleRow.BorderSizePixel=0
    introToggleRow.LayoutOrder=LO()
    Instance.new("UICorner",introToggleRow).CornerRadius=UDim.new(0,7)
    Instance.new("UIStroke",introToggleRow).Color=Color3.fromRGB(45,45,45)
    Instance.new("UIStroke",introToggleRow).Thickness=1
    ...
    introLbl.TextColor3=Color3.fromRGB(240,240,240)
    ...
    introPillBg.BackgroundColor3=State.introEnabled and Color3.fromRGB(190,190,190) or Color3.fromRGB(35,35,35)
    introPillBg.BorderSizePixel=0
    introPillBg.ZIndex=7
    Instance.new("UICorner",introPillBg).CornerRadius=UDim.new(0,10)
    Instance.new("UIStroke",introPillBg).Color=Color3.fromRGB(90,90,90)
    Instance.new("UIStroke",introPillBg).Thickness=1
    ...
    introDot.BackgroundColor3=State.introEnabled and Color3.fromRGB(255,255,255) or Color3.fromRGB(70,70,70)
    introDot.BorderSizePixel=0
    introDot.ZIndex=8
    Instance.new("UICorner",introDot).CornerRadius=UDim.new(0,7)
    ...
    introClk.MouseButton1Click:Connect(function()
        ...
        TS:Create(introPillBg,TweenInfo.new(0.18,Enum.EasingStyle.Quad),{BackgroundColor3=State.introEnabled and Color3.fromRGB(190,190,190) or Color3.fromRGB(35,35,35)}):Play()
        TS:Create(introDot,TweenInfo.new(0.18,Enum.EasingStyle.Back),{
            ...
            BackgroundColor3=State.introEnabled and Color3.fromRGB(255,255,255) or Color3.fromRGB(70,70,70)
        }):Play()
        task.spawn(function()
            if saveConfig then pcall(saveConfig) end
        end)
    ...
    musicRow.BackgroundColor3=Color3.fromRGB(12,12,12)
    musicRow.BorderSizePixel=0
    musicRow.LayoutOrder=LO()
    Instance.new("UICorner",musicRow).CornerRadius=UDim.new(0,7)
    Instance.new("UIStroke",musicRow).Color=Color3.fromRGB(45,45,45)
    Instance.new("UIStroke",musicRow).Thickness=1
    ...
    musicLbl.TextColor3=Color3.fromRGB(240,240,240)
    ...
    musicDisplay.BackgroundColor3=Color3.fromRGB(8,8,8)
    musicDisplay.BackgroundTransparency=0.5
    musicDisplay.Text="Music " .. State.selectedMusic
    musicDisplay.TextColor3=Color3.fromRGB(255,255,255)
    musicDisplay.Font=Enum.Font.GothamBold
    musicDisplay.TextSize=12
    musicDisplay.TextXAlignment=Enum.TextXAlignment.Center
    Instance.new("UICorner",musicDisplay).CornerRadius=UDim.new(0,6)
    Instance.new("UIStroke",musicDisplay).Color=Color3.fromRGB(60,60,60)
    Instance.new("UIStroke",musicDisplay).Thickness=1
    ...
    prevBtn.BackgroundColor3=Color3.fromRGB(18,18,18)
    ...
    nextBtn.BackgroundColor3=Color3.fromRGB(18,18,18)
    ...
    musicInfo.TextColor3=Color3.fromRGB(120,120,120)
    ...
    makeGap(8)
    makeSectionHeader("Speed Bypass")
    makeGap(4)
    ...
    powerRow.BackgroundColor3=Color3.fromRGB(12,12,12)
    powerRow.BorderSizePixel=0
    powerRow.LayoutOrder=LO()
    Instance.new("UICorner",powerRow).CornerRadius=UDim.new(0,7)
    Instance.new("UIStroke",powerRow).Color=Color3.fromRGB(45,45,45)
    Instance.new("UIStroke",powerRow).Thickness=1
    ...
    powerLbl.TextColor3=Color3.fromRGB(240,240,240)
    ...
    Instance.new("UICorner",powerHandle).CornerRadius=UDim.new(0,5)
    ...
    powerInfo.TextColor3=Color3.fromRGB(120,120,120)
    ...
    fl.Text="secret duel hub  ·  v2.0"
    fl.TextColor3=Color3.fromRGB(55,55,55)
...
-- STEAL BAR
local stealBarGui=Instance.new("ScreenGui",PG)
stealBarGui.Name="SecretDuelStealBar"
stealBarGui.ResetOnSpawn=false
stealBarGui.IgnoreGuiInset=true
stealBarGui.DisplayOrder=9

local stealBar=Instance.new("Frame",stealBarGui)
stealBar.Size=UDim2.new(0,220,0,8)
stealBar.Position=UDim2.new(0.5,-110,1,-60)
...
sbTrack.Size=UDim2.new(1,0,1,0)
sbTrack.Position=UDim2.new(0,0,0,0)
sbTrack.BackgroundColor3=Color3.fromRGB(28,28,28)
sbTrack.BackgroundTransparency=0.3
sbTrack.BorderSizePixel=0
Instance.new("UICorner",sbTrack).CornerRadius=UDim.new(0,4)

progressFill=Instance.new("Frame",sbTrack)
...
stealStatusLbl.Size=UDim2.new(0,220,0,16)
stealStatusLbl.AnchorPoint=Vector2.new(0.5,1)
stealStatusLbl.Position=UDim2.new(0.5,0,1,-64)
stealStatusLbl.BackgroundTransparency=1
stealStatusLbl.Text=""
stealStatusLbl.TextColor3=Color3.fromRGB(255,255,255)
stealStatusLbl.Font=Enum.Font.GothamBold
stealStatusLbl.TextSize=11
stealStatusLbl.TextXAlignment=Enum.TextXAlignment.Center
stealStatusLbl.TextTransparency=1

task.spawn(function()
    local dotStep=0
    local dotSteps={"STEAL.","STEAL..","STEAL..."}
    while true do
        task.wait(0.35)
        if State.isStealing then
            dotStep=dotStep%3+1
            stealStatusLbl.Text=dotSteps[dotStep]
            stealStatusLbl.TextTransparency=0
        else
            stealStatusLbl.Text=""
            stealStatusLbl.TextTransparency=1
            dotStep=0
...
local MOB_BTN_SIZE=48
local MOB_PAD=4
local MOB_COLS=2
local MOB_ROWS=math.ceil(#stackDefs/MOB_COLS)
local BOX_W=MOB_COLS*(MOB_BTN_SIZE+MOB_PAD)+MOB_PAD
local BOX_H=MOB_ROWS*(MOB_BTN_SIZE+MOB_PAD)+MOB_PAD

local mobBox=Instance.new("Frame",gui)
mobBox.Name="SecretDuelMobBox"
mobBox.Size=UDim2.new(0,BOX_W,0,BOX_H)
mobBox.Position=UDim2.new(1,-(BOX_W+10),0.5,-(BOX_H/2))
mobBox.BackgroundColor3=Color3.fromRGB(8,8,8)
mobBox.BackgroundTransparency=0
mobBox.BorderSizePixel=0
mobBox.Active=true
mobBox.ZIndex=15
Instance.new("UICorner",mobBox).CornerRadius=UDim.new(0,12)
Instance.new("UIStroke",mobBox).Color=Color3.fromRGB(220,220,220)
Instance.new("UIStroke",mobBox).Thickness=2
makeDraggable(mobBox)
...
mobBgImage.ZIndex=1
Instance.new("UICorner",mobBgImage).CornerRadius=UDim.new(0,12)
...
Instance.new("UICorner",mobBoxOverlay).CornerRadius=UDim.new(0,12)

stackWrappers={}
stackBtnRefs={}

for i,def in ipairs(stackDefs) do
    local col=(i-1)%MOB_COLS
    local row=math.floor((i-1)/MOB_COLS)
    local xPos=MOB_PAD+col*(MOB_BTN_SIZE+MOB_PAD)
    local yPos=MOB_PAD+row*(MOB_BTN_SIZE+MOB_PAD)
    
    local btnFrame=Instance.new("TextButton",mobBox)
    btnFrame.Name="MobBtn_"..def.key
    btnFrame.Size=UDim2.new(0,MOB_BTN_SIZE,0,MOB_BTN_SIZE)
    btnFrame.Position=UDim2.new(0,xPos,0,yPos)
    ...
    btnLbl.Size=UDim2.new(1,-4,1,0)
    btnLbl.Position=UDim2.new(0,2,0,0)
    btnLbl.BackgroundTransparency=1
    btnLbl.Text=def.label
    btnLbl.TextColor3=Color3.fromRGB(220,220,220)
    ...
    btnLbl.ZIndex=18
    btnLbl.TextStrokeTransparency=0.3
    btnLbl.TextStrokeColor3=Color3.new(0,0,0)
    ...
    local function setOn(on)
        btnState=on
        TS:Create(btnFrame,TweenInfo.new(0.15),{BackgroundColor3=on and Color3.fromRGB(210,210,210) or Color3.fromRGB(18,18,18)}):Play()
        TS:Create(bStroke,TweenInfo.new(0.12),{Color=on and Color3.fromRGB(255,255,255) or Color3.fromRGB(55,55,55)}):Play()
        btnLbl.TextColor3=on and Color3.fromRGB(0,0,0) or Color3.fromRGB(220,220,220)
    end
    
    stackBtnRefs[def.key]={setOn=setOn}
    
    btnFrame.MouseButton1Down:Connect(function()
        TS:Create(btnFrame,TweenInfo.new(0.05),{BackgroundColor3=Color3.fromRGB(180,180,180)}):Play()
    end)
    
    btnFrame.MouseButton1Up:Connect(function()
        task.delay(0.06,function()
            TS:Create(btnFrame,TweenInfo.new(0.1),{BackgroundColor3=btnState and Color3.fromRGB(210,210,210) or Color3.fromRGB(18,18,18)}):Play()
        end)
    end)
    ...
    btnFrame.MouseButton1Click:Connect(function()
        ...
        if def.key=="instaReset" then
            performInstantReset()
            TS:Create(btnFrame,TweenInfo.new(0.1),{BackgroundColor3=Color3.fromRGB(0,255,0)}):Play()
            task.delay(0.15,function()
                TS:Create(btnFrame,TweenInfo.new(0.15),{BackgroundColor3=btnState and Color3.fromRGB(210,210,210) or Color3.fromRGB(18,18,18)}):Play()
            end)
            return
        end
        
        if def.key=="tpDown" then
            TS:Create(btnFrame,TweenInfo.new(0.05),{BackgroundColor3=Color3.fromRGB(210,210,210)}):Play()
            task.delay(0.3,function()
                TS:Create(btnFrame,TweenInfo.new(0.1),{BackgroundColor3=Color3.fromRGB(18,18,18)}):Play()
            end)
            doTpDown()
            return
        end
...
local function resetProgressBar()
    ...
    TS:Create(fillGlow,TweenInfo.new(0.2),{Size=UDim2.new(0,0,1,0)}):Play()
end
...
stopDropBrainrot=function()
    ...
    for _,cn in ipairs(_dropConns) do
...
stopMedusaCounter=function()
    for _,c2 in pairs(Conns.anchor) do
    ...
    local function pO(v)
        pcall(function()
            ...
            elseif v:IsA("Decal") or v:IsA("Texture") then
                v.Transparency=1
            elseif v:IsA("Fire") or v:IsA("Smoke") or v:IsA("Sparkles") or v:IsA("ParticleEmitter") or v:IsA("Trail") or v:IsA("Beam") then
                v.Enabled=false
            end
        end)
    end
...
local function executeSteal(prompt)
    ...
    Conns.progress=RunService.Heartbeat:Connect(function()
        ...
        TS:Create(progressFill,TweenInfo.new(0.05),{Size=UDim2.new(prog,0,1,0)}):Play()
        TS:Create(fillGlow,TweenInfo.new(0.05),{Size=UDim2.new(prog,0,1,0)}):Play()
...
loadConfig=function()
    ...
    local cfg
    local ok2=pcall(function() cfg=HS:JSONDecode(raw) end)
    ...
    if cfg.tracersEnabled~=nil then tracersEnabled=cfg.tracersEnabled end
...
local function setupChar(char)
    ...
    if head then
        local oldBB=head:FindFirstChild("SecretDuelBB")
        if oldBB then oldBB:Destroy() end
        
        local bb=Instance.new("BillboardGui",head)
        bb.Name="SecretDuelBB"
        ...
        speedBillLbl.Size=UDim2.new(1,0,0,28)
        ...
        lbl2.Position=UDim2.new(0,0,0,32)
        lbl2.BackgroundTransparency=1
        lbl2.Text="/secretduel"
...
RunService.Heartbeat:Connect(function()
    ...
    if State.antiBatOn then
        ...
        if not holdingBat then
            ...
            local spin = root:FindFirstChild("SecretDuelAntiBat")
            if threat then
                if not spin then
                    local bv = Instance.new("BodyAngularVelocity")
                    bv.Name = "SecretDuelAntiBat"
...
RunService.RenderStepped:Connect(function()
    ...
    pcall(function()
        ...
        if head2 then
            local bb2=head2:FindFirstChild("SecretDuelBB")
            if bb2 then
                local sl=bb2:FindFirstChild("SpeedBillLbl")
                if sl then
...
UIS.InputBegan:Connect(function(inp, gp)
    ...
    if inp.KeyCode == Enum.KeyCode.G then
        State.instantResetOnMedusa = not State.instantResetOnMedusa
        print("Medusa Reset:", State.instantResetOnMedusa and "ON" or "OFF")
    end
end)
...
print("═══════════════════════════════════════════════════════")
print("SECRET DUEL HUB v2.0 - Loaded (All Features)")
print("═══════════════════════════════════════════════════════")
print("  Click SECRET DUEL logo to toggle GUI")
print("  /secretduel in chat to toggle GUI")
print("")
print("  KEYBINDS (Configurable in Keybinds tab):")
print("  Keyboard: Click keybind button then press a key")
print("  Controller: Click 🎮 button then press controller button")
...
print("")
print("  NEW FEATURES:")
print("  🛡️ V1 Anti-Ragdoll - 400 speed boost on ragdoll exit")
print("  🎵 Intro Music System - 9 songs with preview")
print("  🎬 SECRET DUEL HUB Intro Animation - Shows on execute")
print("  ⏱️ Ragdoll Timer - 3-2-1-GO! countdown when ragdolled")
print("  💾 Save Button - Save all settings to config")
print("  🚪 Auto Exit Duel - Auto sends GGS then leaves")
print("  🎮 Controller Support - Full controller keybinding")
print("")
print("  Candy Bat Aimbot Features:")
print("  ⚔️ Sticky targeting - locks onto closest enemy")
print("  ⚔️ Adaptive prediction - predicts enemy movement")
print("  ⚔️ Auto-swing - automatically swings when in range")
print("  ⚔️ Height tracking - jumps to match target height")
print("")
print("  Anti Bat Bypass (Mechanics Tab):")
print("  🛡️ Limits bat speed to prevent flinging")
print("  🛡️ Customizable threshold and cap speed")
print("")
print("  Speed Bypass (Settings Tab):")
print("  ⚡ Toggle on/off with slider")
print("  ⚡ Adjust power from 5,000 to 500,000")
print("")
print("  Mobile Buttons:")
print("  - SECRET DUEL HUB logo in background")
